...
const standardizePhoneNumbers = (arr) => {
  ...
  let join=split.map(a=>a.join(''));
  let split2=join.map(a=>a.split('-'));
  let join2=split2.map(a=>a.join(''));
  let allSplit=join2.map(a=>a.split(''));
  allSplit.forEach(a=>a.splice(0,1));
  allSplit.forEach(a=>a.splice(3,1));
  return allSplit.map(a=>a.join(''));
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
